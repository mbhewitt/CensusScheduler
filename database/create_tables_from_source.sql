...
create table op_shifts (
   select left(s.date,4) year, (case when d.datename is null then 'OffPlaya' else d.datename end) datename, s.date, s.shift, 
        ...
        max(details) details, c.WAPpoints wap_points,concat(s.date,' ',left(s.shift,5)) start_time_lt,concat(s.date,' ',
        right(s.shift,5)) end_time_lt, concat((case when d.datename is null then 'OffPlaya' else d.datename end)," ",c.shortname," ",s.shift) as shiftname,
        c.shortname,false add_shift,false delete_shift, false update_shift 
...
alter table op_shifts add primary key(shift_position_id), add key(shift_id), add key (year,off_playa), add notes longtext;
...
create table op_volunteers (
   select v.shiftboard_id,v.newplaya playa_name,concat(v.first,' ',v.last) world_name,p.email,GREATEST(p.mobilephone,p.homephone,p.workphone) phone,
         ...
         false new_account,false needs_update, 0 new_shiftboard_id,n.score, z.rs_shifts,z.total_shifts 
...
alter table op_volunteers add notes longtext, add location longtext, add emergency_contact longtext, add primary key(shiftboard_id), 
	add key (passcode,shiftboard_id), modify world_name text,modify email text, modify phone text;

insert ignore into op_volunteers (shiftboard_id,playa_name,world_name,email,phone,passcode,account_id,core_crew,new_account,needs_update,
   ...
   select v.shiftboard_id,v.newplaya playa_name,concat(v.first,' ',v.last) world_name,p.email,GREATEST(p.mobilephone,p.homephone,p.workphone)
         ...
         core_crew, false new_account,false needs_update, 0 new_shiftboard_id,n.score, z.rs_shifts,z.total_shifts 
...
alter table op_volunteer_roles add primary key(shiftboard_id,roles), add key(roles), add add_role boolean default false, add delete_role boolean default false;
...
create table op_roles (roles varchar(50) not null, add_role boolean default false, delete_role boolean default false, display boolean default true, primary key(roles));
...
create table op_volunteer_shifts (
   select distinct concat(date,event_code,subject) shift_position_id, shiftboard_id,id shiftboard_shift_id, 
         case when cast(concat(date,' ',right(shift,5)) as datetime)>=now() and noshow='' then 'X' else noshow end noshow, false add_shift,
         false delete_shift, false update_shift from shiftboard2 where left(date,4)=left(date_sub(now(),interval 5 month),4)
   );
