...
-- token here and sets the long-lived signed `peers-device` cookie. Passcode
-- sign-in (/api/sign-in) is refused unless that cookie is present, so
-- participants on the same network / playa internet can never passcode-login.
--
-- Why the columns:
--   token       - opaque random string (base64url) encoded in the QR. Looked up
--                 on claim; PRIMARY KEY so a token maps to exactly one row.
