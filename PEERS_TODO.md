...
- [x] **Same-type back-to-back shift limit** (papabear 2026-07-21, shipped `bf0e10a`): a Squaddie/Shift Lead may hold at most **two consecutive** same-type shifts — a claim forming a run of **3+ back-to-back** same-type shifts is blocked. "Back-to-back" = overlap ≤60 min OR next starts within 30 min of the previous ending (Squaddie chains via 30-min gaps; Lead chains via ≤60-min overlaps). Per type; Coordinators exempt. Enforced server-side next to the cross-type overlap rule in `src/pages/api/shifts/[timeId]/volunteers/index.ts`. (Supersedes the earlier "just block Lead↔Lead overlap" framing.)
