...
- [x] **Back-to-back shift limit (type-agnostic)** (papabear 2026-07-21, shipped `bf0e10a` then `04eb7ea`): a volunteer may hold at most **two consecutive** shifts — a claim forming a run of **3+ back-to-back** shifts is blocked. **Counts Squaddie + Shift Lead together** (a Lead shift then two back-to-back Squaddie shifts = three in a row → blocked); Coordinators exempt. "Back-to-back" = overlap ≤60 min OR next starts within 30 min of the previous ending. Enforced server-side next to the cross-type overlap rule in `src/pages/api/shifts/[timeId]/volunteers/index.ts`. (`bf0e10a` was per-type; `04eb7ea` made it type-agnostic to close the mixed-type loophole.)
