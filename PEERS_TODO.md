...
## Parked / nice-to-have
- [ ] **SQUAD / LEAD shift badges** (papabear 2026-07-21, `9a6d03a`, **on staging only — do NOT ship to prod yet**): arched wordmark PNGs matching the PEERS patch font (Alfa Slab One), shown on the shift calendars (middle-right of the box) and tables (end of Type/Position cell) via `client/src/utils/shiftBadge.ts` (Squaddie→SQUAD, Lead→LEAD, Coordinator→none). papabear approved the look but parked it for his team's feedback on census-ops-test first. Prod is intentionally behind peers-main by this commit — isolate it if an unrelated prod deploy is needed before it's approved. Ship to prod (and un-park) when papabear gives the word.
