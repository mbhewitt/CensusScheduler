...
function buildIcs(args: {
  ...
  const lines = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//PEERS Scheduler//PEERS Shift//EN",
    `METHOD:${args.method}`,
    "CALSCALE:GREGORIAN",
    "BEGIN:VEVENT",
    `UID:${args.uid}`,
    ...
    "LOCATION:PEERS Lab\\, 6:30 & A\\, Black Rock City",
    `STATUS:${status}`,
    "TRANSP:OPAQUE",
    "END:VEVENT",
    "END:VCALENDAR",
...
function renderShiftBody(
  ...
  lines.push(
    "",
    "Location: All PEERS shifts start and end at PEERS Lab at 6:30 & A."
  );
  return lines.join("\n");
}
...
export async function notifyAssignment(
  ...
  const opener = isSelfAction
    ? "You're assigned to the following PEERS shift:"
    : `You were assigned to the following PEERS shift by ${actorName ?? "an administrator"}:`;
  ...
  await enqueueEmail({
    ...
    subject: `PEERS: assigned to ${ctx.position} on ${dayLabel}`,
    bodyText,
    ics: icsContent
      ? {
          filename: `peers-${ctx.position.replace(/\s+/g, "-").toLowerCase()}.ics`,
...
export async function notifyRemoval(
  ...
  if (cause.kind === "self") {
    opener = "You've removed yourself from the following PEERS shift:";
    subject = `PEERS: you dropped ${ctx.position} on ${dayLabel}`;
    calendarLine =
      "A calendar cancellation is attached — your calendar should drop the event automatically.";
  } else if (cause.kind === "shift-canceled") {
    opener = "The following PEERS shift has been canceled:";
    subject = `PEERS: ${ctx.shift_name} on ${dayLabel} canceled`;
    calendarLine =
      "A calendar cancellation is attached — your calendar should drop the event automatically.";
  } else {
    ...
    opener = `You were removed from the following PEERS shift by ${actorName ?? "an administrator"}:`;
    subject = `PEERS: removed from ${ctx.position} on ${dayLabel}`;
    calendarLine =
      "A calendar cancellation is attached — your calendar should drop the event automatically.";
  }
  ...
  await enqueueEmail({
    ...
    ics: icsContent
      ? {
          filename: `peers-${ctx.position.replace(/\s+/g, "-").toLowerCase()}-cancel.ics`,
          content: Buffer.from(icsContent, "utf8"),
        }
      : undefined,
    category:
...
export async function notifyRestoration(
  ...
  const bodyText = [
    greetingFor(ctx),
    "",
    "Good news — a PEERS shift you were on has been un-canceled. Your assignment is back on:",
    "",
    renderShiftBody(ctx, dayLabel, timeLabel),
    "",
  ...
  await enqueueEmail({
    to: ctx.email,
    recipientShiftboardId: shiftboardId,
    subject: `PEERS: ${ctx.shift_name} on ${dayLabel} is back on`,
    bodyText,
    ics: icsContent
      ? {
          filename: `peers-${ctx.position.replace(/\s+/g, "-").toLowerCase()}-restored.ics`,
          content: Buffer.from(icsContent, "utf8"),
        }
      : undefined,
