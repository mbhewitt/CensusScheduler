...
// /api/provision/claim, which sets the trusted peers-device cookie. On success
...
export const ProvisionDevice = () => {
  ...
  useEffect(() => {
    ...
    if (!token) {
      setStatus("error");
      setMessage(
        "No provisioning code found in this link. Re-scan the QR code."
      );
      return;
    }
    let cancelled = false;
    fetch("/api/provision/claim", {
      ...
      .then(async (res) => {
        ...
        if (res.ok) {
          ...
          setMessage(
            data?.message ??
              "Provisioning failed. Ask an admin for a new QR code."
          );
        }
      })
      .catch(() => {
        ...
        setStatus("error");
        setMessage(
          "Network error. Make sure the tablet is online and try again."
        );
      });
    return () => {
      cancelled = true;
