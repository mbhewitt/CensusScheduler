...
// server via /api/auth/device. Used by SignIn (show the passcode form) and the
// header (pick the short tablet idle timeout). Defaults to false until the
// one-shot check resolves, so a normal device never briefly exposes passcode.
